...
void get_status(double p, double t, char* status) {
	...
	else if(t < 34 || t > 37) strcpy(status, check_pressure);
...
int main() {
	...
	while(1) {
		...
		usart_transmit_message(buffer);									// transmit payload
		usart_receive_message(answer);									// receive esp answer
		if(!strcmp(answer, success)) esp_ans_display(success, 3);		// display esp answer
		else esp_ans_display(fail, 3);
		load();															// loading to see esp answer
		...
		usart_receive_message(answer);									// receive esp answer
		esp_ans_display(answer, 4);										// display esp answer
		load();															// loading to see esp answer
	}
	return 0;
}
