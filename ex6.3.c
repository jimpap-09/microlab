...
uint16_t pressed_digit = NO_KEY_PRESSED;
int counter = 0;
char prev_key = '\0';
char prev_digit = '\0';
...
void scan_keypad_rising_edge() {
	...
	if(pressed_keys_tempo != pressed_keys) {
		...
		if(pressed_keys_tempo != NO_KEY_PRESSED)
			counter++;
	}
}
...
int main(void) {
	...
	while (1) {
		...
		if(key != '\0' && key != prev_key) lcd_data(key);

		if(counter == 2) {
			// team 36
			if(prev_digit == '3' && key == '6') {
				// PORTB Leds ON
				PORTB = 0xff;
				// delay 3 sec
				_delay_ms(3000);
				// PORTB Leds OFF for 2 seconds to complete 5 seconds delay
				PORTB = 0x00;
				_delay_ms(2000);
			}
			else {
